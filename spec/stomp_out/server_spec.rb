require 'spec_helper'
require File.join(File.dirname(__FILE__), '..', '..', 'lib', 'stomp_out', 'server_user')
...
describe StompOut::Server do
  ...
  before(:each) do
    @user = StompOut::ServerUser.new
    @options = {}
    @server = StompOut::Server.new(@user, @options)
  end

  context :initialize do
    it "initializes attributes" do
      @server.connected.should == false
  ...
  context :receive_data do
    it "processes frame data" do
      ...
      @user.called.should == [:on_connect, :send_data]
      @user.frame.command.should == "CONNECT"
    end
    ...
    it "reports error to client" do
      @server.receive_data("CONNECT\naccept-version:1.0,1.1,1.2\n\n\000\n").should be true
      @user.called.should == [:send_data, :on_error]
      @user.data.should == "ERROR\ncontent-length:62\ncontent-type:text/plain\nmessage:Missing 'host' header\n" +
                           "\nFailed frame:\n-----\nCONNECT\naccept-version:1.0,1.1,1.2\n\n\n-----\000\n"
      @user.error.message.should == "Missing 'host' header"
    end
  end

  context :send_data do
    it "passes data to user" do
      @server.send_data("CONNECTED\nversion:1.2\nheart-beat:0,0\n\n\000").should be true
      @user.called.should == [:send_data]
      @user.data.should == "CONNECTED\nversion:1.2\nheart-beat:0,0\n\n\000"
    end
        ...
        context :message do
          context "when connected" do
            ...
            it "sends MESSAGE frame to client" do
              ...
              @user.called.should == [:on_connect, :send_data, :on_subscribe, :send_data]
              @user.data.should == "MESSAGE\ncontent-length:5\ncontent-type:text/plain\ndestination:/queue" +
                                   "\nmessage-id:123\nsubscription:1\n\nhello\000\n"
            end
              ...
              context "when ack == #{ack}" do
                if versions == "1.0" && ack != "client-individual"
                  ...
                  it "records specified ack ID but leaves it out of frame" do
                    ...
                    @user.called.should == [:on_connect, :send_data, :on_subscribe, :on_unsubscribe, :on_subscribe, :send_data]
                    @user.data.should == "MESSAGE\ncontent-length:5\ncontent-type:text/plain\ndestination:/queue" +
                                         "\nmessage-id:123\nsubscription:2\n\nhello\000\n"
                  end
        ...
        context :error do
          ...
          context "Exception" do
            ...
            it "indicates to client that this is an internal server error" do
              ...
              @user.error.message.should == "failed"
            end
      ...
      context versions do
        context :receive_connect do
          it "raises ProtocolError if already connected" do
            @server.receive_data("CONNECT#{accept_version}\nhost:stomp\n\n\000\n")
            @server.receive_data("CONNECT#{accept_version}\nhost:stomp\n\n\000\n")
            @user.called.should == [:on_connect, :send_data, :send_data, :on_error]
            @user.error.message.should == "Already connected"
          ...
          it "raises ProtocolError if receipt is requested by client" do
            @server.receive_data("CONNECT#{accept_version}\nhost:stomp\nreceipt:1\n\n\000\n")
            @user.called.should == [:send_data, :on_error]
            @user.error.message.should == "Receipt not permitted"
          ...
          context "with heartbeat" do
            ...
            it "uses user-specified limits when negotiating heartbeat setting and starts heartbeat" do
              flexmock(EM::PeriodicTimer).should_receive(:new).and_return(@timer).twice
              @server = StompOut::Server.new(@user, @options.merge(:min_send_interval => 6000, :desired_receive_interval => 30000))
              @server.receive_data("CONNECT#{accept_version}\nheart-beat:4000,10000\nhost:stomp\n\n\000\n")
              @user.called.should == [:on_connect, :send_data]
          ...
          it "sets server header if user specified server name" do
            @server = StompOut::Server.new(@user, @options.merge(:name => "test", :version => "1.0"))
            @server.receive_data("CONNECT#{accept_version}\nhost:stomp\n\n\000\n")
            @user.called.should == [:on_connect, :send_data]
            @user.data.should =~ /CONNECTED.*\nserver:test\/1.0\n/
          end
          ...
          it "reports connection to user and allows it to be rejected" do
            flexmock(@user).should_receive(:on_connect).with(StompOut::Frame, "test", "secret", "stomp", String).
                and_return(false).once
            @server.receive_data("CONNECT#{accept_version}\nhost:stomp\nlogin:test\npasscode:secret\n\n\000\n")
            @user.called.should == [:send_data, :on_error]
            @user.error.message.should == "Invalid login"
        ...
        context :receive_message do
          context "when connected" do
            ...
            it "raises ProtocolError if destination header is missing" do
              ...
              @user.called.should == [:on_connect, :send_data, :send_data, :on_error]
              @user.error.message.should == "Missing 'destination' header"
            ...
            it "reports message received to user" do
              @server.receive_data("SEND\ndestination:/queue\n\nhello\000\n")
              @user.called.should == [:on_connect, :send_data, :on_message]
              @user.frame.command.should == "SEND"
              @user.destination.should == "/queue"
              @user.message.should == "hello"
              @user.content_type.should == "text/plain"
            end

            it "sends receipt if requested" do
              @server.receive_data("SEND\ndestination:/queue\nreceipt:1\n\nhello\000\n")
              @user.called.should == [:on_connect, :send_data, :on_message, :send_data]
              @user.data.should == "RECEIPT\nreceipt-id:1\n\n\000\n"
            end
          end
          ...
          it "raises ProtocolError if not connected" do
            @server.receive_data("SEND\ndestination:/queue\n\nhello\000\n")
            @user.called.should == [:send_data, :on_error]
            @user.error.message.should == "Not connected"
          ...
          context "when connected" do
            ...
            if version == "1.0"
              it "generates id if there is no id header" do
                @server.receive_data("SUBSCRIBE\ndestination:/queue\n\n\000\n")
                @server.instance_variable_get(:@subscriptions).should == {"/queue" => {:id => "1", :ack => "auto"}}
              end
            else
              it "raises ProtocolError if missing id header" do
                @server.receive_data("SUBSCRIBE\ndestination:/queue\n\n\000\n")
                @user.called.should == [:on_connect, :send_data, :send_data, :on_error]
                @user.error.message.should == "Missing 'id' header"
            ...
            ["auto", "client", "client-individual"].each do |ack|
              if version != "1.0" || ack != "client-individual"
                it "records #{ack} ack setting with subscription" do
                  @server.receive_data("SUBSCRIBE\nack:#{ack}\ndestination:/queue\nid:2\n\n\000\n")
                  @server.instance_variable_get(:@subscriptions).should == {"/queue" => {:id => "2", :ack => ack}}
                end
              else
                it "raises ProtocolError if ack setting is invalid" do
                  @server.receive_data("SUBSCRIBE\nack:#{ack}\ndestination:/queue\nid:2\n\n\000\n")
                  @user.called.should == [:on_connect, :send_data, :send_data, :on_error]
                  @user.error.message.should == "Invalid 'ack' header"
                end
              end
            end
            ...
            it "defaults ack setting to auto" do
              @server.receive_data("SUBSCRIBE\ndestination:/queue\nid:2\n\n\000\n")
              @server.instance_variable_get(:@subscriptions).should == {"/queue" => {:id => "2", :ack => "auto"}}
            end
            ...
            it "reports successful subscribe to user" do
              @server.receive_data("SUBSCRIBE\nack:client\ndestination:/queue\nid:2\n\n\000\n")
              @user.called.should == [:on_connect, :send_data, :on_subscribe]
              @user.frame.command.should == "SUBSCRIBE"
              @user.subscribe_id.should == "2"
              @user.destination.should == "/queue"
              @user.ack_setting.should == "client"
            end
          ...
          context "when connected" do
            ...
            if version == "1.0"
              it "raises ProtocolError if missing both id and destination headers" do
                @server.receive_data("UNSUBSCRIBE\n\n\000\n")
                @user.called.should == [:on_connect, :send_data, :on_subscribe, :send_data, :on_error]
                @user.error.message.should == "Missing 'destination' header"
                @user.data.should == "ERROR\ncontent-length:39\ncontent-type:text/plain\nmessage:Missing 'destination' header\n" +
                                     "\nFailed frame:\n-----\nUNSUBSCRIBE\n\n\n-----\000\n"
              end
              ...
              it "raises ProtocolError if subscription not found" do
                @server.receive_data("UNSUBSCRIBE\ndestination:/queue2\n\n\000\n")
                @user.called.should == [:on_connect, :send_data, :on_subscribe, :send_data, :on_error]
                @user.error.message.should == "Subscription not found"
                @user.data.should == "ERROR\ncontent-length:59\ncontent-type:text/plain\nmessage:Subscription not found\n" +
                                     "\nFailed frame:\n-----\nUNSUBSCRIBE\ndestination:/queue2\n\n\n-----\000\n"
              end

              it "deletes subscription" do
                @server.instance_variable_get(:@subscriptions).should == {"/queue" => {:id => "1", :ack => "client"}}
                @server.receive_data("UNSUBSCRIBE\ndestination:/queue\n\n\000\n")
                @server.instance_variable_get(:@subscriptions).should == {}
              end

              it "report successful unsubscribe to user" do
                @server.receive_data("UNSUBSCRIBE\ndestination:/queue\n\n\000\n")
                @user.called.should == [:on_connect, :send_data, :on_subscribe, :on_unsubscribe]
                @user.subscribe_id.should == "1"
                @user.destination.should == "/queue"
              end

              it "sends receipt if requested" do
                @server.receive_data("UNSUBSCRIBE\nid:1\nreceipt:1\n\n\000\n")
                @user.called.should == [:on_connect, :send_data, :on_subscribe, :on_unsubscribe, :send_data]
                @user.data.should == "RECEIPT\nreceipt-id:1\n\n\000\n"
              end
            else
              it "raises ProtocolError if missing id header" do
          ...
          context "when connected" do
            ...
            if version < "1.2"
              it "raises ProtocolError if missing message-id header" do
                ...
                @user.called.should == [:on_connect, :send_data, :on_subscribe, :send_data, :on_error]
                @user.error.message.should == "Missing 'message-id' header"
              ...
              if version == "1.0"
                it "reports ack to user" do
                  ...
                  @server.receive_data("ACK\nmessage-id:123\n\n\000\n")
                  @user.called.should == [:on_connect, :send_data, :on_subscribe, :send_data, :on_ack]
                ...
                it "sends receipt if requested" do
                  @server.message({"destination" => "/queue", "message-id" => "123"}, "hello")
                  @server.receive_data("ACK\nmessage-id:123\nreceipt:1\n\n\000\n")
                  @user.called.should == [:on_connect, :send_data, :on_subscribe, :send_data, :on_ack, :send_data]
                  @user.data.should == "RECEIPT\nreceipt-id:1\n\n\000\n"
                end
              else
                it "reports ack to user" do
          ...
          context "when connected" do
            ...
            if version == "1.0"
              it "raises ProtocolError if nack not supported" do
                ...
                @user.called.should == [:on_connect, :send_data, :on_subscribe, :send_data, :on_error]
                @user.error.message.should == "Invalid command"
              ...
              it "reports nack to user" do
                @server.message({"destination" => "/queue", "message-id" => "123", "subscription" => "1"}, "hello")
                @server.receive_data("NACK\nmessage-id:123\n\n\000\n")
                @user.called.should == [:on_connect, :send_data, :on_subscribe, :send_data, :on_nack]
                @user.ack_id.should == "1"
              end
              ...
              it "sends receipt if requested" do
                @server.message({"destination" => "/queue", "message-id" => "123", "subscription" => "1"}, "hello")
                @server.receive_data("NACK\nmessage-id:123\nreceipt:1\n\n\000\n")
                @user.called.should == [:on_connect, :send_data, :on_subscribe, :send_data, :on_nack, :send_data]
                @user.data.should == "RECEIPT\nreceipt-id:1\n\n\000\n"
              end
            else
              it "raises ProtocolError if missing id header" do
        ...
        context :receive_begin do
          context "when connected" do
            ...
            it "raises ProtocolError if missing transaction header" do
              ...
              @user.called.should == [:on_connect, :send_data, :on_subscribe, :send_data, :on_error]
              @user.error.message.should == "Missing 'transaction' header"
            ...
            it "raises ProtocolError if transaction already exists" do
              ...
              @server.receive_data("BEGIN\ntransaction:1\n\n\000\n")
              @user.called.should == [:on_connect, :send_data, :on_subscribe, :send_data, :on_error]
              @user.error.message.should == "Transaction already exists"
        ...
        context :receive_commit do
          context "when connected" do
            ...
            it "raises ProtocolError if transaction not found" do
              @server.receive_data("COMMIT\ntransaction:1\n\n\000\n")
              @user.called.should == [:on_connect, :send_data, :on_subscribe, :send_data, :on_error]
              @user.error.message.should == "Transaction not found"
            ...
            it "processes frames in transaction and deletes it" do
              ...
              @user.called.should == [:on_connect, :send_data, :on_subscribe, :on_message, :on_message]
            end
        ...
        context :receive_disconnect do
          context "when connected" do
            ...
            it "reports disconnect to user" do
              ...
              @user.called.should == [:on_connect, :send_data, :on_disconnect]
              @user.frame.command.should == "DISCONNECT"
              @user.reason.should == "client request"
            end

            it "sends receipt if requested" do
              @server.receive_data("BEGIN\ntransaction:1\n\n\000\n")
              @server.receive_data("DISCONNECT\nreceipt:1\n\n\000\n")
              @user.called.should == [:on_connect, :send_data, :on_disconnect, :send_data]
              @user.data.should == "RECEIPT\nreceipt-id:1\n\n\000\n"
            end
          end
    ...
    context :send_frame do
      context "when connected" do
        ...
        it "sends frame to client" do
          ...
          frame.body.should == ""
          @user.called.should == [:on_connect, :send_data, :send_data]
          @user.data.should == "MESSAGE\ndestination:/queue\nmessage-id:123\nsubscription:1\n\n\000\n"
        end
