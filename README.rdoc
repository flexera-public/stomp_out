...
data from the connection and they expect the user of the classes to provide a function
for receiving data that has been STOMP encoded for transmission on the network.

The Client and Server classes support version 1.0, 1.1, and 1.2 of STOMP. The built-in
version negotiation in the protocol determines which is actually applied over a given
connection.
...
There are two sides to the interface: the functions the StompOut::Client[link:lib/stomp_out/client.rb] class exposes
and the callback functions the user of the Client must support. The latter are documented
in the mock StompOut::ClientUser[link:lib/stomp_out/client_user.rb] class used for testing.

The Client exposes the following functions in addition to a constructor:
...
The user of the Client must expose the following callback functions:
...
There are two sides to the interface: the functions the StompOut::Server[link:lib/stomp_out/server.rb] class exposes
and the callback functions the user of the Server must support. The latter are documented
in the mock StompOut::ServerUser[link:lib/stomp_out/server_user.rb] class used for testing.

The Server exposes the following functions in addition to a constructor:
...
The user of the Server must expose the following callback functions:
...
The above callbacks should raise StompOut::ApplicationError[link:lib/stomp_out/errors.rb] for requests
that violate any application specific constraints.

= Example

The gem contains client[link:examples/websocket_client.rb] and server examples of how it can be applied
